...
import { SummonerPreviewCardMastery } from "../SummonerPreviewCardMastery";
import { SummonerPreviewCardDownload } from "../SummonerPreviewCardDownload";

import "../../styles/SummonerPreviewCard.css";
import "../../styles/SummonerPreviewCardDesktop.css";

import "../../scripts/SummonerPreviewCard.js";
          ...
          <div className="row" id="summoner-card-row">
              ...
              <div className="summoner-card-image">
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.target.src = "https://ddragon.leagueoflegends.com/cdn/11.8.1/img/profileicon/29.png";
                  }}
                />
              </div>
              <span className={`summoner-card-name summoner-bg-${player.highestLeague}`}>{player.name}</span>
            </div>
            <div className={`col-3 summoner-border-right-${player.highestLeague}`}>
              {player.summonerRank.map((queue) => (
                <div className="summoner-card-info" key={queue._id}>
                  ...
                  <div className="league-queue-division">
                    ...
                    <div className={`league-division summoner-league-division-${queue.league}`}>
            ...
            <div className="col-1 summoner-card-mastery">
              {player.championMastery.slice(0, 4).map((champion) => (
                <SummonerPreviewCardMastery champion={champion} key={champion._id} />
              ))}
            </div>
          </div>
        </div>
      </div>
      <SummonerPreviewCardDownload player={player} />
    </>
  );
};
